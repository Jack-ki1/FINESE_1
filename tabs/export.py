...
"""
        ...
        try:
            ...
            md += tabulate(df.head(10), headers='keys', tablefmt='pipe')
        except ImportError:
            # Fallback if tabulate is not available
            md += "Sample data not available (requires tabulate package)"
...
# ✅ ZIP BUNDLE CREATOR
# =============================
def _create_zip_bundle(pdf_path: Optional[str], pptx_path: Optional[str], markdown_path: str, df: pd.DataFrame) -> BytesIO:
...
# =============================
def render_export_tab(filtered: pd.DataFrame) -> None:
    """
    ...
    work_df = st.session_state.get('work_df', filtered)
    if work_df is None:
