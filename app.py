...
from utils import reset_app, get_filtered_data, create_kpis, calculate_data_health_score
...
session_keys = {
    "base_df": None,  # Changed from pd.DataFrame() to None to save memory
    "work_df": None,  # Changed from pd.DataFrame() to None to save memory
    "filtered_data": None,  # Changed from pd.DataFrame() to None to save memory
    "data_loaded": False,
    "leaderboard": None,
    "pipeline": None,
    "model_metadata": {},
    "reset_requested": False,
    "homepage_shown": False,
    "target_col": None,
    "problem_type": None,
    "business_goal": "General purpose (balanced)",
    "model_cost": None,
    "change_log": [],
    "theme": "light"  # Add theme to session state with default value
}
...
# =============== GLOBAL FILTERS ===============
# Only import get_filtered_data when needed
from utils import get_filtered_data
filtered = get_filtered_data()

# =============== KPIs ===============
# Only import create_kpis when needed
from utils import create_kpis
with st.container():
    k1, k2, k3, k4, k5 = st.columns(5)
    create_kpis(k1, k2, k3, k4, k5, filtered)
